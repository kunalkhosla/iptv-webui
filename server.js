...
async function buildTasteForProfile(p) {
  ...
  for (const mode of ["movie", "series"]) {
    ...
    const { signal, excludeIds } = tasteSignalFor(state, mode);
    if (signal.length < 3) continue; // not enough history to infer taste
  ...
  const executors = {
    ...
    now_streaming() {
      ...
      for (const [, v] of streams) {
        ...
        // "Still watching?" window matches the idle reaper: live gets the
        // longer LIVE_IDLE_GRACE_MS (a buffering cast stops fetching but
        // is still wanted), VOD the shorter window.
        const graceMs = v.mode === "live" ? LIVE_IDLE_GRACE_MS : 120_000;
...
async function spawnTranscoder(mode, id, quality, offsetSecs, audio, preset, key, diskSel = null, respawn = null, audioTrack = 0) {
  audioTrack = normalizeAudioTrack(audioTrack);
