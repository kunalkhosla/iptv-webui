...
async function prewarmAudioInfoCache() {
...
function streamUrl(mode, id, ext) {
  ...
  return `${PANEL}/${m.pathSeg}/${IPTV_USER}/${IPTV_PASS}/${id}.${e}?_=${Date.now()}`;
}
...
async function buildIndex(mode) {
  ...
  try {
    ...
    let priorSize = 0;
    try {
      const prior = JSON.parse(fs.readFileSync(indexFilePath(mode), "utf8"));
...
app.get("/api/home/:mode(live|movie|series)", (req, res) => {
  ...
  if (mode !== "live") {
    ...
    const lpForMode = lastPlayed[mode] || {};
