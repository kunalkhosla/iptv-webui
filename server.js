...
function scheduleLastPlayedSave(actx = currentAccount()) {
  ...
  const t = setTimeout(async () => {
    ...
    try {
      ...
      const lp = isOwnerAccount(actx) ? lastPlayed : (lastPlayedByAccount.get(key) || { live:{}, movie:{}, series:{} });
...
function getLastPlayedFor(actx) {
  ...
  if (!lp) {
    ...
    try {
      const raw = fs.readFileSync(dataPathFor(actx, "lastPlayed"), "utf8");
      const d = JSON.parse(raw);
      lp = {
        live: d.live && typeof d.live === "object" ? d.live : {},
        movie: d.movie && typeof d.movie === "object" ? d.movie : {},
...
async function clearDiskIndexes(actx = currentAccount()) {
  for (const mode of ["live", "movie", "series"]) {
...
const _ownerIndexes = { live: emptyIndex(), movie: emptyIndex(), series: emptyIndex() };
...
function getIndexesFor(actx) {
  ...
  if (!ix) {
    ix = { live: emptyIndex(), movie: emptyIndex(), series: emptyIndex() };
...
const indexes = new Proxy(_ownerIndexes, {
  ...
  ownKeys() { return ["live", "movie", "series"]; },
...
async function buildAllIndexes(actx = currentAccount()) {
  await Promise.all([
    buildIndex("live", actx),
    buildIndex("movie", actx),
    buildIndex("series", actx),
  ]);
}
...
app.get("/api/transcode/:mode(live|movie|series)/:id/index.m3u8", async (req, res, next) => {
  ...
  const owner = ownerKeyOf(req);
  const killer = () => {
    const t = transcoders.get(transcoderKey(req.params.mode, req.params.id, quality, offsetSecs, audio));
    if (t) { try { t.proc.kill("SIGTERM"); } catch {} }
  };
  const ad = admitStream(owner, req.params.mode, req.params.id, killer, currentAccountKey());
  if (!ad.ok) return sendDisplaced(res);
  try {
    const t = await startOrTouchTranscoder(req.params.mode, req.params.id, quality, offsetSecs, audio);
...
app.delete("/api/user-state/recents/:mode(movie|series|live)/:id", (req, res) => {
  const userState = getProfileState(req.profileId);
  const mode = req.params.mode;
...
app.delete("/api/user-state/recents/:mode(movie|series|live)", (req, res) => {
  const userState = getProfileState(req.profileId);
  const mode = req.params.mode;
  userState.recents[mode] = [];
...
app.get("/api/:mode(live|movie|series)/item/:id", (req, res) => {
  const { mode, id } = req.params;
  const ix = indexes[mode];
...
function transcoderKey(mode, id, quality, offsetSecs = 0, audio = "stereo") {
  const off = Number(offsetSecs) || 0;
  const suffix = off > 0 ? `-t${off}` : "";
  const a = audio === "surround" ? "-surround" : "";
  return `${mode}-${id}-${normalizeQuality(quality)}${a}${suffix}`;
}
...
async function startOrTouchTranscoder(mode, id, quality, offsetSecs = 0, audio = "stereo") {
  quality = normalizeQuality(quality);
  offsetSecs = normalizeOffsetSecs(offsetSecs);
  audio = normalizeAudio(audio);
  const preset = QUALITY_PRESETS[quality];
  const key = transcoderKey(mode, id, quality, offsetSecs, audio);
  ...
  //    works in browsers + any device. Web client always lands here.
  //  - "surround": E-AC3 384k passthroughs all source channels (5.1 /
  //    7.1). ExoPlayer on an Android TV connected to an AVR via HDMI
  //    will bitstream this to the AVR for native multi-channel
  //    playback. If the sink can't decode E-AC3, ExoPlayer downmixes
  //    locally — slight CPU cost, no UX regression. Browsers can't
  //    decode E-AC3 so the web client must NOT request this mode.
  const audioArgs = audio === "surround"
    ? ["-c:a", "eac3", "-b:a", "384k"]
    : ["-c:a", "aac", "-b:a", preset.copy ? "192k" : preset.aBitrate, "-ac", "2"];
  if (preset.copy) {
    // Source passthrough: video unchanged (saves a lot of CPU).
