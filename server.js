...
app.get("/api/transcode/:mode(live|movie|series|disk)/:id/index.m3u8", async (req, res, next) => {
  ...
  const killer = (reason = "admit-displace") => {
    stopTranscoder(transcoders.get(transcoderKey(req.params.mode, req.params.id, quality, offsetSecs, audio, diskSel)), reason);
  };
  const ad = admitStream(owner, req.params.mode, req.params.id, killer, currentAccountKey());
  if (!ad.ok) return sendDisplaced(res);
  try {
    const t = await startOrTouchTranscoder(req.params.mode, req.params.id, quality, offsetSecs, audio, diskSel);
...
app.get("/api/transcode/:mode(live|movie|series|disk)/:id/seg_:n.ts", (req, res) => {
  ...
  const audio = normalizeAudio(req.query.a);
  const diskSel = diskSelFromQuery(req.params.mode, req.query);
  const t = transcoders.get(transcoderKey(req.params.mode, req.params.id, quality, offsetSecs, audio, diskSel));
...
app.use((req, res, next) => {
  ...
  if (cookieProfileOwned) { req.profileId = cookieProfileId; return runNext(); }
...
// Live casts on a marginal WiFi link can rebuffer for minutes: the Cast
// receiver stops fetching segments mid-stall, which to the server is
// indistinguishable from "viewer left". Reaping the transcode then turns
// a recoverable blip into a dead cast — there's nothing left to resume
// into when WiFi recovers. So live gets a longer idle grace in BOTH
// reapers (the streams-map owner reaper AND the transcoder-process
// reaper); the ffmpeg keeps producing to its rolling segment window and
// the receiver catches the live edge when it comes back. VOD keeps the
// short windows — a stalled position there is a genuine freeze, and the
// cap=1 panel slot should free promptly. Env-tunable for the trade-off
// against how long a departed live viewer holds the single slot.
const LIVE_IDLE_GRACE_MS = (() => {
  const n = parseInt(process.env.LIVE_IDLE_GRACE_MS, 10);
  return Number.isFinite(n) && n > 0 ? n : 180_000;
})();
// Single "is this stream still wanted?" idle window, mode-aware. EVERY
// teardown/keepalive decision must use this so they agree on one
// boundary: the idle reapers, the self-heal respawn gate, and the stall
// watchdog. If they disagree (e.g. reaper waits 180s but respawn gives
// up at 90s), a live ffmpeg that exits mid-rebuffer gets torn down in
// the gap and the cast has nothing to resume into — the exact dead-cast
// this grace exists to prevent.
const idleWindowMs = (mode) => (mode === "live" ? LIVE_IDLE_GRACE_MS : TRANSCODE_IDLE_MS);
...
setInterval(() => {
  const now = Date.now();
  for (const [k, v] of streams) {
    // Active live viewers get the long grace so a rebuffer survives; a
    // displaced entry is already dead, so reap it on the short window
    // regardless of mode (its killers already fired; this just clears
    // the tag so the owner can start a fresh stream).
    const limit = (v.mode === "live" && !v.displaced) ? LIVE_IDLE_GRACE_MS : STREAM_IDLE_MS;
...
function transcoderKey(mode, id, quality, offsetSecs = 0, audio = "stereo", diskSel = null) {
  ...
  return `${mode}-${id}-${normalizeQuality(quality)}${a}${suffix}${dsel}`;
}
...
async function startOrTouchTranscoder(mode, id, quality, offsetSecs = 0, audio = "stereo", diskSel = null) {
  quality = normalizeQuality(quality);
  offsetSecs = normalizeOffsetSecs(offsetSecs);
  audio = normalizeAudio(audio);
  const preset = QUALITY_PRESETS[quality];
  const key = transcoderKey(mode, id, quality, offsetSecs, audio, diskSel);
  ...
  const startPromise = spawnTranscoder(mode, id, quality, offsetSecs, audio, preset, key, diskSel);
...
async function spawnTranscoder(mode, id, quality, offsetSecs, audio, preset, key, diskSel = null, respawn = null) {
  ...
  if (MODES[mode].local) {
    ...
    if (HW_ENCODE && !copyVideo && burnIdx == null) args.push("-vaapi_device", VAAPI_DEVICE);
    ...
    if (mode === "live") {
      // Live upstream is flaky: panels stall reads and rotate CDN hosts.
      // Without these the single ffmpeg dies on the first hiccup and the
      // cast drops to IDLE/ERROR. -rw_timeout aborts a stalled segment read
      // (15s, microseconds) so a silent stall errors fast instead of hanging;
      // -reconnect_streamed lets ffmpeg re-open the http connection mid-stream
      // and ride over a transient drop, keeping segment numbering continuous.
      // Do NOT add -reconnect_at_eof here: every HLS segment EOFs normally, so
      // it makes ffmpeg reconnect-loop and never advance (the reverted c5540f0).
      args.push(
        "-rw_timeout", "15000000",
        "-reconnect", "1",
  ...
  ffmpeg.on("exit", (code) => {
    ...
    const recentViewer = now - entry.lastAccess < idleWindowMs(mode);
    ...
    if (!connectFail) { entry.everConnected = true; entry.slotRetries = 0; }
    // Respawn decision:
    ...
    setTimeout(() => {
      ...
      if (entry.stopping || Date.now() - entry.lastAccess > idleWindowMs(mode)) {
        transcoders.delete(key); fs.rmSync(dir, { recursive: true, force: true }); return;
      }
      spawnTranscoder(mode, id, quality, offsetSecs, audio, preset, key, diskSel, { startNumber: nextStart, entry })
        .catch((e) => {
...
setInterval(() => {
  ...
  for (const [key, t] of transcoders) {
    if (now - t.lastAccess > idleWindowMs(t.mode)) {
...
setInterval(() => {
  ...
  for (const [key, t] of transcoders) {
    ...
    // Keep healing a wedged live transcode through the full live grace,
    // not just 90s — a SIGTERM+respawn during a rebuffer rebuilds the
    // rolling segment window so the receiver has fresh segments to catch
    // when WiFi recovers. Past the grace, the idle reaper owns teardown.
    if (now - t.lastAccess > idleWindowMs(t.mode)) continue;
...
app.get(/^\/(live|movie|series|disk)(\/.*)?$/, (_req, res) => {
