...
async function buildIndex(mode, actx = currentAccount()) {
  ...
  try {
    ...
    for (const c of cats) {
      ...
      const added = ix.byId.size - beforeSize;
...
app.get("/api/transcode/:mode(live|movie|series|disk)/:id/index.m3u8", async (req, res, next) => {
  ...
  const owner = ownerKeyOf(req);
  const killer = () => {
    stopTranscoder(transcoders.get(transcoderKey(req.params.mode, req.params.id, quality, offsetSecs, audio, diskSel)));
  };
...
async function spawnTranscoder(mode, id, quality, offsetSecs, audio, preset, key, diskSel = null, respawn = null) {
  ...
  ffmpeg.on("exit", (code) => {
    ...
    const recentViewer = now - entry.lastAccess < TRANSCODE_IDLE_MS;
    const canRespawn = !entry.stopping && mode === "live"
      && recentViewer && entry.restarts.length < MAX_RESTARTS;
    if (!canRespawn) {
    ...
    console.log(`[transcode ${key}] exit ${code} — respawn #${entry.restarts.length} (start_number=${nextStart})${tail ? ": " + tail : ""}`);
...
setInterval(() => {
  ...
  for (const [key, t] of transcoders) {
    ...
    if (now - t.lastAccess > TRANSCODE_IDLE_MS) continue; // no viewer; idle reaper owns it
...
app.post("/api/transcode/stop", express.json(), (req, res) => {
  ...
  if (mode && id) {
    ...
    const prefix = `${mode}-${id}-`;
    for (const [k, t] of transcoders) {
      if (k.startsWith(prefix)) { stopTranscoder(t); killed++; }
    }
  } else {
    for (const t of transcoders.values()) { stopTranscoder(t); killed++; }
  }
