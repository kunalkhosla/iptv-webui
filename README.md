...
- **Multi-profile**, household-wide login — separate favorites / recents /
  watched / progress per profile, sync across the web client and the Android
  TV app on the same profile automatically. Header chip shows the active
  profile and opens a click-to-switch popup from any screen
...
IPTV_PASS=...
APP_USER=admin
APP_PASS=...                  # gate for the web UI itself
TMDB_API_KEY=...              # YOUR OWN key — register at themoviedb.org (free, ~2 min). Optional but strongly recommended.
PROXY_SECRET=...              # optional; stabilises proxy URLs across restarts
IPTV_HOST_FALLBACK=http://backup-panel.example   # optional secondary panel
```

`IPTV_*` are your panel credentials (they end up in stream URLs).
`APP_*` is the Basic Auth login that gates the web UI — set strong since
anyone with these can reach the panel through your server.
...
A GitHub Action builds and pushes `ghcr.io/<owner>/iptv-webui:latest` on
every push to `main`. On the VPS:

```
cd /docker/iptv-webui
