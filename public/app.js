...
function renderRail({ title, items, navId, total, hideable }) {
...
async function openMovie(s) {
  saveScroll();
  // Switch to movie mode so updateUrl() builds a `/movie/...` path
  // rather than inheriting `/series/...` (or whatever tab the user
  // was on before clicking a movie tile from search-all / person
  // credits). Lightweight version of setMode — no rail reload, no
  // home redirect, just the URL-affecting bits.
  if (state.mode !== "movie") {
    state.mode = "movie";
    for (const b of el.modeButtons) b.classList.toggle("active", b.dataset.mode === "movie");
  }
