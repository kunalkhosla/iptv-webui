...
const state = {
  mode: "live",
  query: "",
  modes: { live: emptyModeState(), movie: emptyModeState(), series: emptyModeState() },
  favorites: { live: new Set(), movie: new Set(), series: new Set() },
  ...
  myList:    { live: new Set(), movie: new Set(), series: new Set() },
  ...
  lastPlayed: { live: {}, movie: {}, series: {} },
  sort: {
    live:   { f: "name", dir: "asc" },
    movie:  { f: "name", dir: "asc" },
    series: { f: "name", dir: "asc" },
  },
  playing: null,
  hls: null,
  castSession: null,
  ...
  home: { live: null, movie: null, series: null },
  homeFetching: { live: false, movie: false, series: false },
...
state.filter = {
  onboarded: false,
  groups: { live: new Set(), movie: new Set(), series: new Set() },
};
...
const el = {
  ...
  modeButtons: document.querySelectorAll("#modes .mode"),
...
async function loadCategoryStreams(mode, catId) {
  ...
  if (m.byCat.has(key)) return m.byCat.get(key);
  const r = await fetch(`/api/${mode}/streams?category_id=${encodeURIComponent(catId)}`);
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  const list = await r.json();
  m.byCat.set(key, list);
  return list;
}
      ...
      for (const mode of MODES) {
        ...
        if (!ix.ready) {
          ...
          for (const [k, list] of groups) {
            ...
            if (!existing || !existing.length) state.modes[mode].byCat.set(k, list);
          }
...
function filteredStreams(mode, list) {
  if (!filterIsActive(mode)) return list;
  const allowedCatIds = new Set(filteredCategories(mode).map(c => String(c.category_id)));
  return list.filter(s => allowedCatIds.has(String(s.category_id)));
}
...
async function fetchHomeRails(mode) {
  ...
  try {
    ...
    console.warn("fetchHomeRails failed:", e.message);
  ...
  // fallback was up.
  if (state.view === "home" && state.mode === mode) renderRails();
}
...
async function selectCategory(id, opts = {}) {
  ...
  showGrid();
  if (id !== PSEUDO.RECENTS && id !== PSEUDO.FAVS && id !== PSEUDO.MY_LIST && id !== PSEUDO.ALL) {
...
function continueWatchingItems(mode) {
  ...
  if (mode === "movie") {
    const entries = Object.entries(state.progress)
      ...
      .slice(0, 25);
    return entries
      .map(([k]) => all.find(s => `movie:${s.id}` === k))
      .filter(Boolean);
  }
  // series: surface the series whose last-played episode has progress
  const entries = Object.entries(state.lastEpisode)
    .map(([seriesId, ep]) => {
      const prog = state.progress[`series:${ep.episode_id}`];
      return { seriesId, when: (prog && prog.t) || ep.when || 0, ep, prog };
    })
    .filter(e => e.when)
    .sort((a, b) => b.when - a.when)
    .slice(0, 25);
  return entries
    .map(e => all.find(s => String(s.id) === String(e.seriesId)))
    .filter(Boolean);
}
...
// Score an item by quality (rating) with a recency boost from the
// panel's `added` timestamp. Range roughly 0 to ~10.5 for top tier
// (high rating + newly added). Items missing rating or added still get
// a usable score so the pool doesn't go empty.
function heroScore(item) {
  const rating = parseFloat(item.rating);
  const ratingScore = Number.isFinite(rating) && rating > 0 ? Math.min(10, rating) : 4;
  // `added` is a unix-second timestamp from the panel for movies and
  // most series. Recency bonus tapers off across a year.
  const added = Number(item.added);
  let recencyBonus = 0;
  if (Number.isFinite(added) && added > 0) {
    const ageDays = (Date.now() / 1000 - added) / 86400;
    if (ageDays < 30) recencyBonus = 1.5;
    else if (ageDays < 90) recencyBonus = 1.0;
    else if (ageDays < 365) recencyBonus = 0.5;
  } else {
    // Fall back to year for items missing `added` (most series).
    const y = parseInt(item.year, 10);
    if (Number.isFinite(y)) {
      const now = new Date().getFullYear();
      if (now - y <= 1) recencyBonus = 1.2;
      else if (now - y <= 3) recencyBonus = 0.7;
      else if (now - y <= 6) recencyBonus = 0.3;
    }
  }
  return ratingScore + recencyBonus;
}

function buildHeroCandidates() {
  const mode = state.mode;
  // Hero is for *discovery*, not resume. Continue-Watching items
  // were eating every slot for weeks (Kapil Sharma on every reload);
  // they're explicitly excluded here, the Continue Watching rail
  // still surfaces them just below. Same change on the server in
  // /api/home — see the heroPool block in server.js.
  const adultFree = (s) => s && !isHiddenFromRecents(mode, s);
  const cwIds = new Set(continueWatchingItems(mode).map(s => s.id));

  const cats = filteredCategories(mode).filter(c => !HIDDEN_FROM_RECENTS.test(c.category_name));
  const seen = new Set(cwIds);
  const pool = [];
  for (const c of cats) {
    const items = categoryItems(mode, c.category_id, 50);
    for (const it of items) {
      if (seen.has(it.id)) continue;
      if (!adultFree(it)) continue;
      seen.add(it.id);
      pool.push(it);
    }
  }
  pool.sort((a, b) => heroScore(b) - heroScore(a));
  const topRanked = pool.slice(0, 30);
  shuffleInPlace(topRanked);

  return topRanked.slice(0, 15);
}
...
function renderHero() {
  ...
  _heroItems = buildHeroCandidates();
...
function renderRails() {
  ...
  if (mode !== "live") {
    const rec = gate(recentItems(mode));
    if (rec.length) {
      const footer = document.createElement("div");
      footer.className = "rails-footer";
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "rails-footer-link";
      btn.textContent = `Recently played (${rec.length}) →`;
      btn.onclick = () => selectCategory(PSEUDO.RECENTS);
      footer.appendChild(btn);
      el.rails.appendChild(footer);
    }
  }
  ...
  // Continue Watching (movies + series only)
  const cwRaw = continueWatchingItems(mode);
  const cw = gate(cwRaw);
  if (cw.length) rails.push({ title: "Continue Watching", items: cw, navId: null, total: cwRaw.length });
...
function renderRail({ title, items, navId, total }) {
  ...
  if (navId) {
    ...
    more.onclick = () => selectCategory(navId);
  ...
  for (const s of items) track.appendChild(channelCard(s, { reason: title }));
...
const _browseQuickFilter = { movie: new Set(), series: new Set() };
...
function renderGuideTabBody() {
  ...
  for (const ch of list) frag.appendChild(buildGuideRow(ch, { noEpg: _guideTab === "without" }));
  el.guideRows.appendChild(frag);
  setupGuideObserver();
  for (const track of el.guideRows.querySelectorAll(".guide-track")) appendNowLineTo(track);
}
...
function buildGuideRow(ch, { noEpg = false } = {}) {
  ...
  if (ch.icon) {
    const img = new Image();
    img.referrerPolicy = "no-referrer";
    img.onload = () => { left.querySelector(".ch-logo").style.backgroundImage = `url("${ch.icon}")`; };
    img.src = ch.icon;
  }
  left.title = ch.name;
...
function currentList() {
  ...
  if (m.activeCatId === PSEUDO.RECENTS) {
    const all = flatStreams();
    return state.recents[state.mode]
      .map(id => all.find(s => s.id === id))
      .filter(Boolean)
      .filter(s => !isHiddenFromRecents(state.mode, s));
  }
  if (m.activeCatId === PSEUDO.FAVS) {
    return flatStreams().filter(s => state.favorites[state.mode].has(s.id));
  }
  if (m.activeCatId === PSEUDO.MY_LIST) {
    return flatStreams().filter(s => state.myList[state.mode].has(s.id));
  }
  if (m.activeCatId === PSEUDO.ALL) {
    return filteredStreams(state.mode, flatStreams()).slice(0, 600);
  }
...
function applySort(list) {
  ...
  if (state.query || ms().activeCatId === PSEUDO.RECENTS) return list;
...
async function renderUnifiedSearch(q) {
  ...
  const total = data.movie.length + data.series.length + data.live.length;
  if (!total) {
...
function renderGrid() {
  ...
  if (el.gridCount) {
    el.gridCount.textContent = list.length === 1 ? "1 item" : `${list.length.toLocaleString()} items`;
  }
  syncSortUI();
  el.grid.innerHTML = "";
  if (!list.length) {
...
function channelCard(s, opts = {}) {
  ...
  const variant = state.mode === "live" ? "live" : "poster";
  ...
  // Capture the mode at card-creation time (via opts.mode) so the
  // click handler doesn't depend on whatever state.mode happens to
  // be when the user actually taps. Without this, opening the
  // search-all view from the Movies tab and clicking a result in the
  // *Series* section routed the click through openMovie() with a
  // series id — which then 4xx'd at /api/stream because the panel's
  // /movie/<series_id>.mp4 path doesn't exist.
  const cardMode = opts.mode || state.mode;
  card.onclick = (e) => {
    if (e.target.classList.contains("star")) return;
    if (cardMode === "series") openSeries(s);
    else if (cardMode === "movie") openMovie(s);
    else play("live", s);
  };
  ...
  // Apply a background-image URL to the .logo, swap initials away.
  ...
  if (s.icon) setLogoImg(s.icon);
  // For movies/series, queue a TMDB upgrade once this card scrolls
  // into view. The IntersectionObserver scopes the lookup so the home
  // rails don't fire dozens of TMDB calls for off-screen rows.
  if (state.mode !== "live") {
    card.dataset.posterId = String(s.id);
    card.dataset.posterMode = state.mode;
  ...
  let progPct = null;
  if (state.mode === "movie") {
    const p = state.progress[`movie:${s.id}`];
  ...
  // "Why is this here?" badge — small dim line under the title that
  // names the rail / category / chip context that surfaced the tile.
  // Helps debug filter rules without diving into DevTools.
  if (opts.reason && state.mode !== "live") {
  ...
  if (state.mode === "series") {
    ...
    const prog = state.progress[`movie:${s.id}`];
    const ts = state.lastPlayed.movie?.[s.id];
    if (prog) {
      const sub = document.createElement("div");
      sub.className = "sub";
...
function sendProgress() {
  ...
  const pos = el.video.currentTime;
...
// scrolls within 300px of the viewport, fetch its TMDB poster and swap
// the .logo background to it (only if TMDB returned an actual poster).
// Cards with `data-poster-id` are observed by `observePosterCard()`.
...
async function resolveStreamUrl(mode, id, ext, anchorSecs) {
  ...
  const qs = anchorSecs && anchorSecs > 0 ? `?t=${Math.floor(anchorSecs)}` : "";
...
async function play(mode, item, label, forceExt, useTranscode) {
  saveScroll();
  clearProgressTracking();
  ...
  state.playing = {
    ...
    ext: forceExt || pickExt(mode, item),
    transcode: !!useTranscode || forceQuality,
    quality: forceQuality ? userQuality : null,
    // Source-side anchor for transcoded movies. 0 = playing from
    // the start (default). Non-zero after the user has scrubbed
    // past the encoded edge and we re-spawned ffmpeg with -ss.
    transcodeAnchorSecs: 0,
    // Full movie length, populated below via /api/movie/info.
    // The scrubber treats this as authoritative for the denominator
    // when transcoding, so 14min encoded ≠ "the movie is 14 min".
    fullDurationSecs: 0,
  };
  ...
  fetchFullDurationSecs(mode, item.id).then(secs => {
    if (state.playing && state.playing.item === item) {
      state.playing.fullDurationSecs = secs || 0;
    }
  });
  ...
  const ext = state.playing.ext;
  const resolved = await resolveStreamUrl(mode, item.id, ext);
  ...
  if (state.playing.transcode) {
    ...
    url = resolved.transcode + (state.playing.quality ? `&q=${state.playing.quality}` : "");
    isHls = true;
  ...
  // Resume from saved position (movies + series episodes only — live and
  // transcoded streams are sliding-window so seeking back has no meaning).
  if (mode !== "live" && !state.playing.transcode) {
    const key = `${mode}:${item.id}`;
    const saved = state.progress[key];
    attachProgressTracking(key);
    if (saved && Number.isFinite(saved.p) && saved.p > 30) {
      const seek = () => {
        const dur = el.video.duration;
        const pos = saved.p;
...
// --- Movie detail (reuses the series modal's hero + close UI) -------
async function openMovie(s) {
  saveScroll();
  // Stash the movie on state.openMovie so the play button can find it.
  state.openMovie = s;
...
function playEpisode(seriesItem, ep, sn) {
  ...
  closeSeries();
  play("series", { id: ep.id, name: title, container: ext },
...
async function applyPath() {
  ...
  while (i < parts.length) {
    ...
    if (tok === "cat" && parts[i + 1]) {
      ...
      if (typeof syncSearchClearVisibility === "function") syncSearchClearVisibility();
      showGrid();
      el.gridTitle.textContent = `Search: ${state.query}`;
      el.grid.classList.toggle("poster", state.mode !== "live");
      renderGrid();
      contextApplied = true;
      i += 2;
      ...
      let item = ms().streams.find(s => s.id === id);
      if (!item) {
        for (const list of ms().byCat.values()) {
...
function openFilterModal({ firstRun = false } = {}) {
  ...
  _filterDraft = {
    live:   new Set(state.filter.groups.live),
    movie:  new Set(state.filter.groups.movie),
    series: new Set(state.filter.groups.series),
  };
...
function syncSearchClearVisibility() {
  ...
  el.searchClear.hidden = !hasValue;
...
async function reanchorTo(targetSecs) {
  ...
  let url = resolved.transcode + (p.quality ? `&q=${p.quality}` : "");
...
function refreshScrubState() {
  ...
  const isTranscode = !!state.playing.transcode;
  elLiveBadge.hidden = !isLive;
  elScrubTrack.closest("#player-scrub-wrap").hidden = false;
  el.player.classList.toggle("scrub-disabled", isLive || isTranscode);
  if (isLive) {
...
el.seriesPlayBtn.onclick = () => {
  if (state.openMovie) {
    ...
    closeMovie();
    play("movie", { id: m.id, name: m.name, container: m.container || "mp4" }, m.name);
    return;
  }
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "/" && document.activeElement !== el.search) {
    ...
    if (!el.filterModal.hidden && !el.filterClose.hidden) closeFilterModal();
...
el.video.addEventListener("ended", () => sendProgress());
